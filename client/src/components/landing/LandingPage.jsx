...
export function LandingPage() {
  ...
  return (
    <div className={`min-h-screen flex flex-col justify-between relative overflow-hidden transition-colors duration-300 ${isDark ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
      {/* Background glowing gradients */}
      <div className={`absolute -top-40 -left-40 w-96 h-96 rounded-full blur-3xl pointer-events-none ${isDark ? 'bg-cyan-500/15' : 'bg-cyan-400/10'}`} />
      <div className={`absolute top-1/3 -right-40 w-96 h-96 rounded-full blur-3xl pointer-events-none ${isDark ? 'bg-indigo-500/15' : 'bg-indigo-400/10'}`} />
      <div className={`absolute -bottom-40 left-1/3 w-96 h-96 rounded-full blur-3xl pointer-events-none ${isDark ? 'bg-violet-500/15' : 'bg-violet-400/10'}`} />
      ...
      <header className="relative z-10 max-w-7xl mx-auto w-full px-6 py-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className={`text-lg font-bold tracking-tight flex items-center gap-1.5 ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className={`inline-flex items-center justify-center w-10 h-10 rounded-xl border transition-colors ${isDark ? 'bg-slate-900/80 border-slate-800 text-slate-200 hover:bg-slate-800' : 'bg-white border-slate-200 text-slate-700 hover:bg-slate-100 shadow-sm'}`}
          >
      ...
      <main className="relative z-10 max-w-5xl mx-auto px-6 py-12 text-center space-y-8 flex-1 flex flex-col justify-center">
        <div className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border text-xs font-semibold mx-auto backdrop-blur-md ${isDark ? 'bg-cyan-950/60 border-cyan-500/30 text-cyan-300' : 'bg-cyan-50/90 border-cyan-200 text-cyan-700'}`}>
          <Sparkles className="w-3.5 h-3.5 text-cyan-400" />
          <span>Next-Gen Executive Email Management</span>
        </div>

        <h1 className={`text-4xl sm:text-6xl font-extrabold tracking-tight max-w-4xl mx-auto leading-tight ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>
      ...
      {/* Footer */}
      <footer className={`relative z-10 border-t py-6 text-center text-xs ${isDark ? 'border-slate-900 text-slate-500' : 'border-slate-200 text-slate-500'}`}>
